...
The save/gameplay model may keep compact ASCII-like internal tags for compatibility. Rendered gameplay should use the Unicode visual set for terrain, objects, UI chrome, and effects, while enemy and item cells use readable letter glyphs.
...
A fast dagger fighter using Energy, internal combo points, poison setup, burst finishers, and smoke-assisted mobility. The current implementation labels the Rogue dungeon resource as Energy, regenerates it during dungeon turns, shows class-aware Rogue dungeon skill help with Energy/CP and rank-scaled values, and implements Backstab, Venom Edge, Eviscerate, and Smoke Step. Rogue on-hit skill effects only apply when the attack hits. Eviscerate is marked locked in dungeon help and cannot be cast while its rank is 0; once learned, it spends combo points on any valid cast, including a miss. The skill tree screen is class-aware: Rogues see Daggers, Venom, and Smoke branches with Backstab, Venom Edge, Smoke Step, and locked passive unlock rows for Eviscerate, Rupture, and Slip Away until their starter rank requirements are met; once unlocked, those passives appear as selectable rank 0/5 upgrades. Eviscerate upgrades require Backstab rank 2 and start at rank 0/5 until the first skill point is allocated, Rupture unlocks at Venom Edge rank 2, and Slip Away unlocks at Smoke Step rank 2. Rupture starts inactive and extends Venom Edge poison duration by rank once unlocked. Slip Away starts inactive and adds smoke-protection dodge once unlocked. Smoke Step spends Energy after the player chooses an explicit direction, dashes 1-2 cardinal tiles along a clear path to an open destination, resolves normal landing-tile interactions, briefly improves defense with rank-scaled smoke dodge from Smoke Step plus unlocked Slip Away, and enables an empowered Backstab on the next player action. Rogues use Energy instead of mana potions and can equip Rogue bucklers, but not Warrior shields.
...
- Locked/gated skills that have not been allocated into start at rank 0/5; their first skill point raises them to rank 1/5 and activates their effects.
- The skill tree displays locked passive prerequisites inline under the starter skill with a lock marker and current rank progress; locked passives do not also appear as selectable rank rows until their starter reaches the required rank.
...
Weapon, off-hand/shield, helm, chest armor, gloves, boots, belt, amulet, ring 1, and ring 2 are implemented as functional equipment slots. Charm inventory slots remain future scope.
...
- Current implementation status: the player bag, merchant sell inventory, and stash all render as ratatui grids with cursor movement and selected-item detail panels. Occupied bag and stash grid cells color their bracket outline by item rarity while the focused item label uses the same cursed-violet cursor color as selection borders, pulsing between normal and bold violet on timed redraw ticks. Stash keeps one visible cursor by drawing the cursed-violet grid cursor only on the active bag-or-stash side; the inactive side keeps its selection state for transfers/details without showing a second cursor. Inventory and stash containers use the shared gothic palette: muted charcoal-violet borders by default, antique-gold titles, and cursed-violet focus borders only for the active side. The inventory screen keeps the bag grid content-sized, shows selected bag-item stats and equipped comparison lines in a Details panel, and shows a Character panel to the right with all equipped slots arranged as a humanoid body. Tab switches inventory focus between Bag and Character; WASD/arrow keys move the active bag cursor or body-slot cursor. Rings equip into ring 1 when it is empty, otherwise ring 2 when it is empty, otherwise replace ring 1. The merchant sell screen uses the same grid, details, and equipped-comparison layout, adding the selected item's sell value to Details and keeping its separate Equipped comparison panel. Comparison lines list the current item, direct weapon damage/crit or armor/dodge/speed deltas, green increases, red decreases, and cannot-equip requirements when selected gear is locked. Stash shows the bag grid, content-sized stash grid, and active-side details in one screen, letting details expand into spare width and using a stacked details pane on narrower terminals so the `4 x 4` bag and `8 x 8` stash remain visible.
- Current dungeon state includes a `ground_items` list of positioned `GroundItem` records. New dungeon floors start with no ground items; monster deaths from direct attacks, damage-over-time effects, thorns, and class effect damage all roll regular monster loot, and monster, chest, and boss loot that cannot fit in the bag is preserved on the source tile as visible `!` ground loot. A single item on the player tile can be picked up with `g=pickup` or by walking over it when inventory space exists. When multiple items are on the player tile, or the bag is full, `g` opens a responsive ratatui ground-loot picker that lists the items, shows selected item details, allows picking up one item when space exists, and allows discarding selected ground loot. Picking up or discarding from the picker spends a dungeon turn. Walking over loot only auto-picks a single item when inventory space exists; multi-item and full-bag cases leave the loot on the ground and log the picker prompt. If boss reward loot falls to ground, the completed dungeon is retained instead of immediately returning to town so the reward stays accessible; remaining enemies are cleared, boss-floor completion rewards are not paid a second time from the retained clear state, and the player can leave after resolving loot or by using the boss-floor stairs once the boss is defeated. Dropping an inventory item inside a dungeon places it on the player tile; dropping in town is disallowed and leaves the inventory unchanged.
- The player bag is a ratatui screen with a cursor-selectable bag grid, selected item details, and a selectable Character equipment panel; the panels sit side-by-side on wide terminals and stack on narrower terminals.
...
- Stairs activate automatically when the player steps onto them, except unresolved ground loot on the same tile stops further automatic interactions so full-bag boss rewards cannot be skipped accidentally.
...
Sockets are found on dropped equippable gear: weapons, armor, shields/offhands, helms, gloves, boots, belts, amulets, and rings. Starter gear, vendor gear, and other fixed basic items do not roll sockets. The blacksmith cannot add sockets.
...
Gems are normal inventory items found as rare dungeon drops. Each gem type grants exactly one flat stat, and gems are slot-agnostic: any gem can go into any socketed equippable gear item. Gems are drop-only and cannot be upgraded or combined.
...
- JSON save files for readability during development.
- `saves/profile.json` tracks the last played character ID.
- `saves/characters/<character-id>.json` stores one complete character per file.

MVP save/load behavior:

- Auto-save the active character after every player action.
...
- Escape should consistently go back from submenus or return from dungeon to town. Main town uses `q` to save and quit and `c` to open character selection/creation. The first floor of each act can be abandoned to town before every monster is killed; this early escape does not award completed-floor rewards such as Herb Garden herbs. Stairs to deeper floors and retreats from later floors still require clearing the floor.
- Load the last played character automatically on startup when `saves/profile.json` points to a valid per-character save. If the profile is missing or points to a missing character but per-character saves exist, load the first valid saved character and refresh the profile. If no character saves exist, the legacy `saves/save.json` is migrated into the new profile/per-character layout when possible; otherwise character creation is shown.
- Character stashes are per-character and remain inside each character save rather than being shared account-wide.
...
Rogues start with a dagger, light armor, no visible offhand equipped, and 2 lesser health potions.

Mana does not passively regenerate by waiting during dungeon exploration. Mana is restored by drinking mana potions, such as lesser mana potions, by returning to town, or through Sorceress Arcane Recovery on kills.
...
- Lesser mana potions: restore 35% of maximum mana for mana-using classes. Rogues use Energy, cannot use mana potions, and class-aware consumable rolls avoid mana potions for Rogues. The dungeon potion hotkey can drink a mana potion when HP is full and mana is missing.
- Herbs: non-bag alchemy currency. Completing the Herb Garden project grows 1-3 herbs after each completed dungeon floor. Completing the Distillery project unlocks potion crafting: 3 herbs for a lesser health potion, and 3 herbs for a lesser mana potion for mana-using classes.
- Gold

### MVP Equipment Interaction

The inventory screen shows currently equipped weapon, armor, offhand/shield, helm, gloves, boots, belt, amulet, and two rings in a right-side Character panel shaped like a humanoid body. The current target design is a ratatui grid plus body panel: the bag starts at `4 x 4`, shows empty cells, uses a cursor to select cells, and shows item details in a Details panel. Pressing Tab switches focus between the bag and character equipment; WASD/arrow keys move the active cursor through bag cells or body slots. Pressing Enter while the bag is focused equips gear or uses consumables and swaps old non-empty equipped gear back into inventory; empty equipment slots display as `Nothing equipped` and are not inserted into the bag when filled. Character-focused Enter or drop commands are informational only in this pass, leaving equipment changes routed through bag item actions. Pressing `o` in the inventory menu sorts the bag immediately without spending a dungeon turn, grouping carried items by type, rarity, item level, value, and name. When opened during a dungeon run, successful bag equip/use actions spend and resolve a dungeon turn while keeping the inventory screen open. Full-bag replacement gear equips are allowed because removing the selected carried item frees the cell reused for the old gear. Weapon damage should come from the equipped weapon. Armor, shields, helms, gloves, boots, belts, amulets, rings, and their socket bonuses affect armor, dodge, speed, and other socket-granted character totals. Rogues can equip Rogue bucklers, but cannot equip Warrior shields. Dropping a bag item in a dungeon places it on the ground instead of deleting it. Blacksmith upgrade menu actions remain focused on the equipped weapon, armor, and shield in this pass; empty equipment placeholders such as a Rogue's empty offhand cannot be upgraded, and salvaging new armor-like slots grants armor shards.
...
- Randomly generated dropped equippable gear can roll empty sockets; gems can drop from floor 3 onward, the Socket Bench manages free gem insertion, removal, and replacement across equipped and carried socketed gear, and Opal socket bonuses increase variable monster and chest gold.
- Inventory shows simple comparisons versus currently equipped gear in the Details panel when a bag item is selected, while the separate Character panel stays visible to show all equipped slots. Increased direct weapon damage/crit or armor/dodge/speed deltas are green, decreased deltas are red, and cannot-equip requirement text appears there when relevant.
...
- Lesser health potions restore 15% of maximum health; lesser mana potions restore 35% of maximum mana.
- Herbs do not take inventory space. The Distillery UI displays the current herb count and available potion recipes.
- Mana does not passively regenerate by waiting; it restores from mana potions, town returns, or Sorceress Arcane Recovery on kills.
