...
- Current dungeon state includes a `ground_items` list of positioned `GroundItem` records. New dungeon floors start with no ground items; monster and chest loot that cannot fit in the bag is preserved on the source tile until later pickup UI is added. Boss rewards are the exception because boss defeat returns to town and clears the dungeon: boss reward and boss gem drops always go directly into the bag, expanding bag dimensions just enough when possible. If a capped `8 x 8` bag is completely full, the boss reward path may add emergency overflow rows beyond the normal purchasable cap so `ItemGrid` still has enough capacity and the reward is not lost. Dropping an inventory item inside a dungeon places it on the player tile; dropping in town still deletes the item.
...
- The current implementation renders the parent town and dungeon screens with ratatui. Legacy ANSI submenu screens such as
  inventory, merchant, stash, projects, attributes, and skills must temporarily release ratatui's raw-mode ownership while
