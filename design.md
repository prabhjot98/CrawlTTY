...
- Load the last played character automatically on startup when `saves/profile.json` points to a valid per-character save. If no character saves exist, the legacy `saves/save.json` is migrated into the new profile/per-character layout when possible; otherwise character creation is shown.
