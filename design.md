...
The inventory screen shows currently equipped weapon, armor, offhand/shield, helm, gloves, boots, belt, amulet, and two rings. The current target design is a ratatui grid: the bag starts at `4 x 4`, shows empty cells, uses a cursor to select cells, and shows item details in a right side panel. Pressing Enter equips gear or uses consumables and swaps old non-empty equipped gear back into inventory; empty equipment slots display as `Nothing equipped` and are not inserted into the bag when filled. When opened during a dungeon run, successful equip/use actions spend and resolve a dungeon turn while keeping the inventory screen open. Full-bag replacement gear equips are allowed because removing the selected carried item frees the cell reused for the old gear. Weapon damage should come from the equipped weapon. Armor, shields, helms, gloves, boots, belts, amulets, rings, and their socket bonuses affect armor, dodge, speed, and other socket-granted character totals. Rogues can equip Rogue bucklers, but cannot equip Warrior shields. Dropping an item in a dungeon places it on the ground instead of deleting it. Blacksmith upgrade menu actions remain focused on the equipped weapon, armor, and shield in this pass; salvaging new armor-like slots grants armor shards.
...
- Inventory shows simple comparisons versus currently equipped gear in a separate Equipped panel to the right of Details on wide terminals and stacked below Details on narrow terminals. Increased direct weapon damage/crit or armor/dodge/speed deltas are green, decreased deltas are red, and cannot-equip requirement text appears there when relevant.
